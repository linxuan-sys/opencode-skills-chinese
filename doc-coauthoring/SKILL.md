...
解释这种方法有助于确保文档在他人阅读时效果良好（包括当他们将其粘贴到 Claude 中时）。询问他们是否想尝试此工作流程或更喜欢自由形式工作。

如果用户拒绝，自由形式工作。如果用户接受，进入阶段 1。

## 阶段 1：上下文收集

**目标：** 缩小用户知道的和 Claude 知道的之间的差距，使后续能够提供智能指导。
...
- 使用适当的集成读取当前状态
- 检查没有 alt 文本的图像
- 如果存在没有 alt 文本的图像，解释当其他人使用 Claude 理解文档时，Claude 将无法看到它们。询问他们是否想生成 alt 文本。如果是，请求他们将每个图像粘贴到聊天中以生成描述性 alt 文本。
...
**如果集成可用**（例如，Slack、Teams、Google Drive、SharePoint 或其他 MCP 服务器），提到可以直接使用这些来拉入上下文。

**如果未检测到集成且在 Claude.ai 或 Claude 应用中：** 建议他们可以在 Claude 设置中启用连接器，以允许直接从消息应用和文档存储中拉入上下文。
...
告诉他们可以用简写回答（例如，"1: 是，2: 见 #channel，3: 不因为向后兼容"），链接到更多文档，指向要阅读的频道，或者只是继续信息转储。任何对他们最有效的方式。
...
**如果可以访问 artifacts：**
使用 `create_file` 创建 artifact。这给 Claude 和用户一个可以工作的脚手架。
...
**目标：** 用全新的 Claude（无上下文污染）测试文档，验证它对读者是否有效。
...
询问读者 Claude 答错或遇到困难的内容。表明打算修复这些差距。
